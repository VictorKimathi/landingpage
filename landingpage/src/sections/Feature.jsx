          ...
          <div className="relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md-g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            {features.map(({ id, icon, caption, title, text, button }) => (
              ...
              >
                ...
                <p className="mb-5 max-md:mb-6 max-md:h5">{caption}</p>
                <h2 className="mb-7 max-w-400 h3 text-p4 max-md:mb-6 max-md:h5">
                  {title}
                </h2>
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              <div className="w-full absolute bg-s3/20 top-[38%] left-0 right-0 z-10 h-[1px]" />
              {details.map(
                ({ id, icon, title } /* Corrected map expression */) => (
